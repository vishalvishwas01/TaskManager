...
function Navbar({ searchQuery, setSearchQuery, currentDate, setCurrentDate}) {
  ...
  return (
    <div className='w-full bg-amber-50 px-6 md:px-20 py-4 flex flex-wrap gap-4 md:gap-10  items-center shadow-md justify-start 2xl:justify-start sm:justify-center'>
      ...
      <div className='flex-grow min-w-[200px] max-w-xl 2xl:ml-[200px] ml-0'><input type='text' value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder='Search title here' className='w-full h-10 px-4 rounded-2xl shadow-sm text-sm bg-white outline-none'/></div>
      <div className='min-w-[160px] flex-shrink-0 border-2 border-red-500 rounded-md px-2 py-1 flex items-center'><input type='date' value={currentDate} onChange={(e)=>setCurrentDate(e.target.value)} className='w-full outline-none text-sm text-gray-700 bg-transparent placeholder:uppercase'/></div>
    </div>
  );
}
